...
Static, no-build site: **every root `.html` is a live URL** (listed in `sitemap.xml`, linked
internally, regenerated by the workflows in `.github/`). Pages must stay at their current paths —
moving one breaks the URL, the sitemap, and the automations. Three logical zones:

```
── 🌐 SITE (served — these paths ARE the public URLs) ──────────────
├── index.html                 # TCG selector (Pokemon / One Piece)
├── search.html advanced.html collection.html deck-builder.html scan.html
├── market.html metrics.html stats.html top-cards.html trade.html
├── favorites.html goals.html new-releases.html contribute.html submit.html
├── add-price.html admin-photos.html api.html backup.html changelog.html
├── donate.html legal.html settings.html ai-work-tax.html 404.html
├── pokemon/     # index + sets/ (12 guides) + data/ + images/
├── onepiece/    # index + sets/ + images/ (grails, findables, packs)
├── guides/      # 10 buyer guides + images/
├── tools/       # ev-calculator, rip-or-keep
├── admin/       # quality, reports
└── calendar/    # release calendar + events

── 🎨 ASSETS & PLATFORM (root-required) ────────────────────────────
├── base.css  components/     # shared JS/CSS (auth, header, footer, price-utils)
├── icons/  favicon.*
├── manifest.json  sw.js       # PWA (offline)
├── robots.txt  sitemap.xml  llms.txt
├── firebase.json  .firebaserc  firestore.rules   # hosting / DB rules
├── package.json  package-lock.json
└── ai-tasks.json              # AI Work Tax queue (loaded by contribute.html)

── ⚙️ WORKSHOP (dev/ops — NOT served) ──────────────────────────────
├── scripts/     # price fetchers, deal-hunter.py, metrics
├── data/        # price caches, ledgers  (deal-hunter reports gitignored)
├── docs/        # ARCHITECTURE, PRICING, DATABASE_COVERAGE, COMPETITOR_ANALYSIS + archive/
└── .github/workflows/          # 10 automations (price updates, sitemap, set-page gen)

── 📦 PERSONAL TOOL (gitignored, not on the site) ──────────────────
└── sealed-deals/   # sealed booster-box deal hunter (Mercari/Yahoo scanner + 定価 sources)

── 📄 docs at root ── AGENTS.md  CLAUDE.md  DESIGN.md  README.md  LICENSE
```
